...
public class JavaFundamentals {
    ...
    private static boolean isTextContainingTheProperCharacters(String text) {
        // TODO: Implement function here
        boolean hasTheProperCharacters = false;
        char[] goodCharacters = {'a', 'b', 'c'};
        char[] badCharacters = {'x', 'y', 'z'};

        if (text != null) {
            for (char i : goodCharacters) {
                String checkCharacter = String.valueOf(i);
                if (text.contains(checkCharacter)) {
                    hasTheProperCharacters = true;
                }
            }
            for (char j : badCharacters) {
                String checkCharacter = String.valueOf(j);
                if (text.contains(checkCharacter)) {
                    hasTheProperCharacters = false;
                }
            }
        }
        return hasTheProperCharacters;
    }
    ...
    private static int getGreater(int a, int b) {
        ...
        if (a != b) {
            theProperNumber = Math.max(a, b);
        } else {
            theProperNumber = a;
        }
        return theProperNumber;
    }
    ...
    private static Integer getTheGreatest(int[] numbers) {
        ...
        if (numbers == null) {
            respMessage = "The parameter was null.";
            greatestNum = null;
        } else if (numbers.length == 0) {
            respMessage = "The parameter was an empty array.";
            greatestNum = 0;
        } else {
            ...
            for (int j : numbers) {
                if (greatestNum <= 0) {
                    respMessage = "Every numbers was non-positive.";
                } else if (j < 0) {
                    respMessage = "Not every number was positive.";
                } else {
                    respMessage = "";
                }
            }
    ...
    private static int getIndexOfFirstAppearanceOfCharInString(String text, char character) {
        ...
        if (text != null) {
            for (int i = 0; i < text.length(); i++) {
                char indexedCharacter = text.charAt(i);
                String checkedCharacter = Character.toString(indexedCharacter);
                String comparedCharacter = Character.toString(character);
                if (checkedCharacter.equals(comparedCharacter)) {
                    correctedValue = i + 1;
                    break;
                }
            }
    ...
    private static void printWhetherStringContainsCharacter(String text, char character) {
        ...
        int correctedValue = getIndexOfFirstAppearanceOfCharInString(text, character);
        String goodTextWithCharacter = "The " + text + " text contains the " + character + " character, first at this index: " + correctedValue + ".";
        String badTextWithCharacter = "The " + text + " text doesn't contain the " + character + " character.";

        if (text == "") {
            System.out.println("The '' text doesn't contain the " + character + " character.");
        } else if (correctedValue > 0) {
            System.out.println(goodTextWithCharacter);
        } else {
            System.out.println(badTextWithCharacter);
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("1) Sum of nonPositiveNumbers = " + getSum(nonPositiveNumbers));
